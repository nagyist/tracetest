...
import {IOpenAPIValues, IPostmanValues, ITriggerService, TDraftTestForm, TRequestAuth} from 'types/Test.types';
...
interface IPostmanTriggerService extends ITriggerService {
  valuesFromRequestOpenAPI(
    requests: RequestDefinitionExtended[],
    variables: VariableDefinition[],
    identifier: string
  ): undefined | Partial<IOpenAPIValues>;
  valuesFromRequest(
    requests: RequestDefinitionExtended[],
    variables: VariableDefinition[],
    identifier: string
...
const Postman = (): IPostmanTriggerService => {
  return {
    ...
    valuesFromRequestOpenAPI(requests, variables, identifier) {
    ...
    updateFormOpenAPI(requests, variables, identifier, form) {
      const input = this.valuesFromRequestOpenAPI(requests, variables, identifier);
      if (input) {
        form.setFieldsValue(input);
      }
    },
    ...
    getRequestsFromOpenAPI(collection: any) {
      try {
        ...
        const flatMap = Object.keys(collection.paths).flatMap(pathKey =>
          Object.keys(collection.paths[pathKey]).map((key: any) => {
            const path = collection.paths[pathKey][key];
            return {
              ...
              url: `${collection.schemes[0]}://${collection.host}${pathKey}`,
            };
          })
        );
        console.log(flatMap);
        ...
        if (['apikey', 'basic', 'bearer'].includes(request?.auth.type)) {
          ...
          if (request.auth.type === 'basic') {
            return {
              ...
              basic: {
                ...
                password: this.substituteVariable(
                  variables,
                  authParameters?.find(({key}) => key === 'password')?.value
                ),
              },
            };
          }
          if (request.auth.type === 'bearer') {
            return {
              type: this.translateType(request.auth.type),
              bearer: {
                token: this.substituteVariable(
                  variables,
                  authParameters?.find(({key}) => key === 'token')?.value || ''
                ),
              },
            };
          }
